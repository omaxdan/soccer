    ...
    <main className="space-y-4" style={{ maxWidth: 760, margin: '0 auto', padding: 16 }}>
      ...
      {teams.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-2" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {teams.map((t) => (
            <li key={t.id}>
              <Link href={routes.team(t)} className="panel" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, textDecoration: 'none', color: 'inherit' }}>
