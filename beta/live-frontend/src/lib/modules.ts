...
// ── Market profiles ──────────────────────────────────────
// Which modules bear on which market, and how strongly they align. This is a
// GROUPING of readings that already exist — no market is scored, nothing is
// recommended. It answers "which market profile does the historical evidence
// speak to most clearly", not "what should be backed".

export interface MarketProfile {
  market: string;
  /** Modules that produced a directional reading for this market. */
  contributors: ModuleReading[];
  alignment: "Strong" | "Moderate" | "Weak" | "Conflicting" | "No signal";
  color: string;
  sentence: string;
}

const MARKET_MODULES: { market: string; keys: ModuleKey[]; frame: string }[] = [
  {
    market: "Match winner",
    keys: ["form_gap", "confidence", "rest", "home_away", "readiness", "giant_killer"],
    frame: "outright result",
  },
  {
    market: "Draw no bet",
    keys: ["form_gap", "rest", "home_away", "consistency"],
    frame: "result with the draw removed",
  },
  {
    market: "Goals",
    keys: ["league_goals", "btts_fatigue", "weather"],
    frame: "total goals",
  },
  {
    market: "Both teams to score",
    keys: ["btts_fatigue", "clean_sheet", "league_goals"],
    frame: "both sides scoring",
  },
];

export function buildMarketProfiles(readings: ModuleReading[]): MarketProfile[] {
  const byKey = new Map(readings.map((r) => [r.def.key, r]));

  return MARKET_MODULES.map(({ market, keys, frame }) => {
    const contributors = keys
      .map((k) => byKey.get(k))
      .filter((r): r is ModuleReading => r != null && r.status !== "inactive");

    const supports = contributors.filter((r) => r.status === "supports");
    const against = contributors.filter((r) => r.status === "contradicts");

    let alignment: MarketProfile["alignment"];
    if (contributors.length === 0) alignment = "No signal";
    else if (supports.length > 0 && against.length > 0) alignment = "Conflicting";
    else if (supports.length >= 3) alignment = "Strong";
    else if (supports.length >= 1) alignment = "Moderate";
    else alignment = "Weak";
    ...
    const named = (rs: ModuleReading[]) => rs.map((r) => r.def.name).join(", ");
    const sentence =
      alignment === "No signal"
        ? `No module produced a reading bearing on the ${frame}.`
        : alignment === "Conflicting"
          ? `Evidence divides: ${named(supports)} align while ${named(against)} run counter, so the ${frame} profile is unsettled.`
          : alignment === "Weak"
            ? `Modules bearing on the ${frame} all read neutral — no direction emerges.`
            : `Alignment on the ${frame} comes from ${named(supports)}.`;

    return { market, contributors, alignment, color, sentence };
  });
}
