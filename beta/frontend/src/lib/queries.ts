...
export async function getInactiveMatches() {
  // BETA: fresh-DB guard — with no tracked competitions, an unfiltered
  // query would return UNTRACKED matches (and hammer the DB at build time).
 ...
 */
/**
 * BETA v2 — truncation-proof rewrite (audit Phase 3).
 *
 * V1 issued 5 queries; two (form_history and travel_load for ALL ~323
 * tracked teams) exceeded PostgREST's silent 1000-row cap — form pills and
 * travel figures were wrong for any team without a very recent fixture.
 *
 * V2: 3 bounded queries, none exceeding ~350 rows. Form pills read from
 * team_intelligence.last_5_results (precomputed by the beta pipeline —
 * migration 023); travel from travel_load_km (always existed here); trend
 * baseline scoped to a single snapshot date found with a 1-row query.
 *
 * Rule: list endpoints read only one-row-per-team tables. "N rows per team
 * across all teams" cannot be expressed safely in PostgREST — it must be
 * precomputed by the pipeline.
 *
 * Signature preserved (limit param retained for callers) — note the server
 * caps any single response at 1000 rows regardless of the value passed.
 */
export async function getTeamIntelligenceList(limit = 1000): Promise<TeamIntelRow[]> {
  const teamIds = await getTrackedTeamIds();

  const q = supabase.from('team_intelligence')
    .select(`team_id, readiness_score, form_index, congestion_score, rest_days_avg,
  ...
  // Trend baseline: the single most recent snapshot date ≥5 days old —
  // found first (1 row), then fetched for all teams (≤1 row per team).
  ...
  const [standingsRes, baselineDateRes] = await Promise.all([
    ...
    supabase.from('team_intelligence_history').select('snapshot_date')
  ...
  if (baselineDate) {
    const { data: baseline } = await supabase
      ...
      .in('team_id', ids);
    const baseMap = new Map((baseline ?? []).map((b: any) => [b.team_id, b.readiness_score]));
    for (const r of intelRows as any[]) {
      const prev = baseMap.get(r.team_id);
      trendMap.set(r.team_id, prev != null && r.readiness_score != null
  ...
  return (intelRows as any[]).map((r) => {
    ...
    const pills = ((r.last_5_results ?? '') as string).split('')
      .filter((c: string): c is 'W' | 'D' | 'L' => c === 'W' || c === 'D' || c === 'L');
    return {
