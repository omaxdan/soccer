...
export const metadata = { title: 'Postponed & Cancelled' };
...
export default async function InactiveMatchesPage() {
  // BETA: null = data layer failed (render explicit notice), [] = genuinely
  // no postponed/cancelled matches. Distinct states per the Phase 4 audit.
  const matches = await getInactiveMatches().catch(() => null);
  if (matches === null) {
    return (
      <div style={{ padding: 32, textAlign: 'center', color: COLORS.muted, fontSize: 13 }}>
        Match status data couldn&apos;t be loaded — this is a data-layer issue, not an empty schedule. Refresh to retry.
      </div>
    );
  }
