...
export interface ApiDeps {
  ...
  readonly getFixturesByDate: (date: string) => Promise<unknown | null>;
}
...
const productionDeps: ApiDeps = {
  ...
  getFixturesByDate: (date) => withConnection(API_ROLE, (tx: PoolClient) => getFixturesByDate(tx, date)),
};
...
export type Route =
  ...
  | { kind: 'fixturesByDate'; date: string }
...
export function resolveRoute(method: string | undefined, pathname: string): Route {
  ...
  const fixturesByDate = pathname.match(/^\/api\/v2\/fixtures\/([^/]+)$/);
  ...
  if (editionTemporalPerformance) {
    const id = decodeURIComponent(editionTemporalPerformance[1]);
    return isValidId(id) ? { kind: 'editionTemporalPerformance', id } : { kind: 'badRequest' };
  }
  ...
  if (teamPlayerObservations) {
    const id = decodeURIComponent(teamPlayerObservations[1]);
    return isValidId(id) ? { kind: 'teamPlayerObservations', id } : { kind: 'badRequest' };
  }
  if (teamPerformanceSignals) {
  ...
  if (playerTemporalPerformance) {
    ...
    return isValidId(id) ? { kind: 'playerTemporalPerformance', id } : { kind: 'badRequest' };
  }
  if (player) {
...
/** Parse the Team Player Observation Index query filters. Whitelisted: optional edition
 *  scope and asOf (default now, strict `<`). teamId is the path param; editionId is a
...
/** Parse the Team Performance Signals query filters. Whitelisted: optional edition scope
 *  and asOf (default now, strict `<`). Venue splits are computed internally (Season Home/
 *  Away), not a query param; windows are the fixed Last5/Previous5/Season contract. */
export function parseTeamPerformanceSignalsOptions(searchParams: URLSearchParams): TeamPerformanceSignalsOptions {
  const editionRaw = searchParams.get('edition');
  const editionId = editionRaw && isValidId(editionRaw) ? editionRaw : null;
  const asOfRaw = searchParams.get('asOf');
  let asOf: Date | undefined;
...
/** Parse the Edition Temporal Performance query filters. Whitelisted: asOf only
 *  (default now, strict `<`). Edition is the path param; no free-form window builder. */
export function parseEditionTemporalPerformanceOptions(searchParams: URLSearchParams): EditionTemporalPerformanceOptions {
  const asOfRaw = searchParams.get('asOf');
  let asOf: Date | undefined;
  if (asOfRaw) { const d = new Date(asOfRaw); if (!Number.isNaN(d.getTime())) asOf = d; }
  return { asOf };
...
export async function handleRequest(req: IncomingMessage, res: ServerResponse, deps: ApiDeps): Promise<void> {
  ...
  try {
    switch (route.kind) {
      ...
      case 'fixturesByDate': {
        // A valid date always yields a body (empty countries[] when no fixtures) → 200,
        // never 404. Null only occurs for a malformed date, already rejected as badRequest.
        const body = await deps.getFixturesByDate(route.date);
        return body ? sendJson(res, 200, body) : sendJson(res, 400, { error: 'invalid_date' });
      }
      ...
      case 'editionTemporalPerformance': {
        const body = await deps.getEditionTemporalPerformance(route.id, parseEditionTemporalPerformanceOptions(url.searchParams));
        return body ? sendJson(res, 200, body) : sendJson(res, 404, { error: 'edition_not_found' });
      }
      ...
      case 'teamPlayerObservations': {
        const body = await deps.getTeamPlayerObservations(route.id, parseTeamPlayerObservationsOptions(url.searchParams));
        return body ? sendJson(res, 200, body) : sendJson(res, 404, { error: 'team_not_found' });
      }
      case 'teamPerformanceSignals': {
