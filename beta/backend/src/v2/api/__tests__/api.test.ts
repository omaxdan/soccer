...
describe('v2 api · id validation and routing', () => {
  ...
  test('resolveRoute maps method + path to intent', () => {
    ...
    assert.deepEqual(resolveRoute('GET', '/api/v2/teams/18/attributes'), { kind: 'teamAttributes', id: '18' });
    assert.deepEqual(resolveRoute('GET', '/api/v2/teams/abc/attributes'), { kind: 'badRequest' });
    assert.deepEqual(resolveRoute('POST', '/api/v2/teams/18/attributes'), { kind: 'methodNotAllowed' });
...
describe('v2 api · HTTP layer over injected seams (no database)', () => {
  ...
  before(async () => {
    const deps: ApiDeps = {
      ...
      getTeamAttributes: async (id) => (id === '7' ? { team: { id: '7', name: 'T', slug: 't' }, scope: { type: 'edition', editionId: '42', label: 'edition 42' }, asOf: '2026-09-20T00:00:00.000Z', context: 'current edition, completed matches to date', sample: { completedFixtures: 20, classificationFloor: 8 }, insufficientSample: false, strengths: [{ key: 'goals_per_match', label: 'Goals scored per match', type: 'strength', level: 'TOP_QUARTILE', direction: 'HIGHER_IS_MORE', qualityOrientation: 'HIGHER_IS_BETTER', evidence: { signalKey: 'goals_per_match', signalValue: 1.76, benchmark: { rank: 3, teams: 20, percentile: 0.9, median: 1.26, q1: 1.08, q3: 1.48 }, teamFixtures: 20 } }], weaknesses: [], tendencies: [], benchmarkVersion: 'result-edition-cumedist-v1', provenance: { source: 'teamBenchmark', reconstructed: true, immutable: false } } : null),
  ...
  it('GET team player-observations → 200 observed-player index; unexposed team → 404', async () => {
    const res = await fetch(`${base}/api/v2/teams/7/player-observations`);
    assert.equal(res.status, 200);
    const body = await res.json() as any;
    assert.equal(body.team.id, '7');
    assert.equal(body.scope.competition, 'all');
    assert.equal(body.playerCount, 1);
    assert.equal(body.players[0].player.id, '9');
  ...
  it('GET team attributes → 200 with governed strengths; unexposed team → 404', async () => {
    const res = await fetch(`${base}/api/v2/teams/7/attributes?edition=42`);
    assert.equal(res.status, 200);
    const body = await res.json() as any;
    assert.equal(body.team.id, '7');
    assert.equal(body.scope.type, 'edition');
    assert.equal(body.insufficientSample, false);
    assert.equal(body.strengths[0].level, 'TOP_QUARTILE');
    assert.equal(body.strengths[0].qualityOrientation, 'HIGHER_IS_BETTER');
    assert.doesNotMatch(JSON.stringify(body), /VERY_STRONG|VERY_WEAK/);
    const miss = await fetch(`${base}/api/v2/teams/8/attributes?edition=42`);
    assert.equal(miss.status, 404);
    assert.deepEqual(await miss.json(), { error: 'team_not_found' });
  });
