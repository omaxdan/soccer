...
CREATE TABLE public.team_betting_intelligence (
  ...
  updated_at timestamp with time zone DEFAULT now(),
  sample_confidence text DEFAULT 'LOW'::text,
...
CREATE TABLE public.user_profiles (
  ...
  display_name text,
  role text NOT NULL DEFAULT 'user'::text CHECK (role = ANY (ARRAY['user'::text, 'support'::text, 'moderator'::text, 'admin'::text, 'owner'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  avatar_url text,
  provider text,
  ...
  CONSTRAINT user_profiles_suspended_by_fkey FOREIGN KEY (suspended_by) REFERENCES auth.users(id)
);
...
CREATE TABLE public.admin_actions (
  ...
  action text NOT NULL CHECK (action = ANY (ARRAY['role_changed'::text, 'suspended'::text, 'unsuspended'::text, 'plan_upgraded'::text, 'plan_downgraded'::text, 'plan_cancelled'::text, 'plan_extended'::text, 'plan_reset'::text, 'note_added'::text])),
  detail text,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT admin_actions_pkey PRIMARY KEY (id),
  CONSTRAINT admin_actions_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES auth.users(id),
...
CREATE TABLE public.user_notes (
  ...
  note text NOT NULL,
  flag text CHECK (flag IS NULL OR (flag = ANY (ARRAY['suspicious'::text, 'chargeback'::text, 'support_priority'::text, 'vip'::text]))),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT user_notes_pkey PRIMARY KEY (id),
  CONSTRAINT user_notes_target_user_id_fkey FOREIGN KEY (target_user_id) REFERENCES auth.users(id),
  CONSTRAINT user_notes_author_id_fkey FOREIGN KEY (author_id) REFERENCES auth.users(id)
);
CREATE TABLE public.match_intelligence_watch (
  match_id bigint NOT NULL,
  confidence_band text,
  confidence_score numeric,
  readiness_gap numeric,
  ...
  CONSTRAINT match_intelligence_watch_match_id_fkey FOREIGN KEY (match_id) REFERENCES public.matches(id)
);