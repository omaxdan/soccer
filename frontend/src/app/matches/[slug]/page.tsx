...
export default function MatchPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const [hI, aI, hF, aF, hFix, aFix, hT, aT, hS, aS, hUp, aUp] = await Promise.all([
        ...
        setData({ match, homeIntel: hI, awayIntel: aI, homeForm: hF, awayForm: aF, homeFix: hFix, awayFix: aFix, homeTravel: hT, awayTravel: aT, homeSquad: hS, awaySquad: aS, homeUp: hUp, awayUp: aUp });
  ...
  const { match, homeIntel, awayIntel, homeForm, awayForm, homeFix, awayFix, homeSquad, awaySquad, homeUp, awayUp } = data;
  ...
  // Compute signals — falls back to team-level baseline data when
  // match_intelligence hasn't been computed for this specific match yet,
  // same pattern as the readiness gauges above. Previously this was gated
  // entirely on `intel` existing (`intel ? compute(...) : []`), even though
  // most of the inputs already correctly used homeIntel/awayIntel — meaning
  // a match with zero match_intelligence rows showed ZERO signals even when
  // both teams had complete current baseline data, which is the exact gap
  // that was reported.
  ...
  const signals = hasEnoughForSignals ? computeMatchSignals({
    ...
    home_travel_distance_km: intel?.home_travel_distance_km, // no clean team-level
    away_travel_distance_km: intel?.away_travel_distance_km, // equivalent — left as-is
    ...
    // Squad/player fields — populated once sync:squads:v2 has run for these teams
  ...
  // gap/congestion/rest-day inputs are estimated from team baselines rather
  // than the full match-specific formula (no opponent-strength, home-
  // advantage, or motivation components factored into the baseline numbers).
  const signalsAreBaselineOnly = hasEnoughForSignals && !intel;

  // Summary banner signals
  ...
  const TeamColumn = ({ team, intel: ti, form, fix, squad, upcoming }: any) => (
    <Card>
            ...
            {(isDone || isLive) ? (
              ...
              <div>
                ...
                {intel ? (
                  ...
                  // Fallback: compute a rough gap from each team's own current
                  // baseline when match_intelligence isn't ready yet. Clearly
                  // labeled as an estimate — this is NOT the same calculation
                  // as the real match-specific formula (no opponent-strength,
                  // home-advantage, or motivation components factored in).
                  <div style={{ marginTop:8 }}>
      ...
      {tab === 'Overview' && (
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            <TeamColumn team={match.home_team} intel={homeIntel} form={homeForm} fix={homeFix} squad={homeSquad} upcoming={homeUp} />
            <TeamColumn team={match.away_team} intel={awayIntel} form={awayForm} fix={awayFix} squad={awaySquad} upcoming={awayUp} />
          </div>
        </div>
      )}
                  ...
                  <table style={{ width:'100%' }}>
                    ...
                    <tbody>
                      {group.map((s, i) => {
                        ...
                        // Free tier: blur rows 4+ in betting signals per spec
