...
// Sources:
//   team_intelligence: form_index, congestion_score, travel_fatigue_score,
//     last_5_points, squad_depth_score, injury_burden_score,
//     squad_stability_score, active_competitions, rest_days_avg
//   match_intelligence: home/away_readiness, readiness_gap, congestion_factor,
//     home/away_rest_days, travel distances
...
export interface MatchSignalInput {
  ...
  // Squad/player fields from team_intelligence — now wired in:
...
export function computeMatchSignals(m: MatchSignalInput): Signal[] {
  ...
  // Squad/player data — present when sync:squads:v2 has run for these teams
  const homeDepth    = m.home_squad_depth ?? null;
  const awayDepth    = m.away_squad_depth ?? null;
  const homeInjury   = m.home_injury_burden ?? null;   // higher = more injured (0–100)
  const awayInjury   = m.away_injury_burden ?? null;
  const homeStab     = m.home_squad_stability ?? null; // higher = more stable
  ...
  // ── 1X2 ──────────────────────────────────────────────────────────────────
  {
    ...
    // Squad signals — additive when data available
    if (homeInjury != null && awayInjury != null) {
      const injDiff = awayInjury - homeInjury; // positive = away more injured
  ...
  // ── COMPETITION MARKETS ───────────────────────────────────────────────────
  {
    ...
    // Rotation Pressure — now uses real squad_stability_score when available
    {
      let sig = 'No signal'; let dir: 'home' | 'away' | 'neutral' = 'neutral'; let str = 2;
      const driv: string[] = [];

      if (hasSquadData && homeStab != null && awayStab != null) {
        const stabDiff = homeStab - awayStab; // positive = home more stable
  ...
  // ── SQUAD DEPTH ADVANTAGE (NEW — uses squad data when available) ──────────
  ...
  // ── KEY INJURY BURDEN (NEW — uses injury data when available) ────────────
  if (hasSquadData && homeInjury != null && awayInjury != null) {
    const injDiff  = awayInjury - homeInjury; // positive = away more hurt
  ...
  // ── HALF-TIME MARKETS ─────────────────────────────────────────────────────
  {
    ...
    // HT/FT — now uses squad stability when available
    {
...
export function groupSignals(signals: Signal[]) {
  return {
    '1x2':         signals.filter(s => s.group === '1x2'),
    'goals':       signals.filter(s => s.group === 'goals'),
    'competition': signals.filter(s => s.group === 'competition'),
    'halftime':    signals.filter(s => s.group === 'halftime'),
    'cards':       signals.filter(s => s.group === 'cards'),
  };
}
