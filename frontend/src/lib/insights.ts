...
// A transparent, informational tally of which side leads in how many
// tracked categories — NOT a blended predictive score. Deliberately does
// NOT: normalize into z-scores and sum them, apply invented weights or
// "amplification" multipliers, apply penalty modifiers, or output a
// "winner" classification (Heavy Dominance / Volatility Trap / etc). Every
// number here is directly traceable to a category already shown in detail
// elsewhere on this page (CategorizedComparison, the versatility bars in
// Lineups, the Key Player Battle cards) — this is a glance-level count of
// "how many boxes does each side check", nothing is fabricated to look
// more authoritative than a simple tally actually is.
//
// Categories counted (each contributes at most one lead to one side):
//   - The up to 11 baseline comparison rows (Readiness, Form Index,
//     Congestion, Strength Rating, Venue Advantage, Goals Scored/Conceded,
//     Squad Stability, Squad Depth, Injury Impact, Predicted Goals) —
//     using the SAME edge/polarity logic as CategorizedComparison, not a
//     re-derivation, so the tally can never disagree with what's shown below.
//   - Up to 3 area-versatility zones (DEF/MID/FWD) — only counted where
//     BOTH sides have a non-null percentage. A team with 0% in a zone
//     while the opponent has 75% simply loses that one tally point, same
//     weight as any other category. No structural penalty multiplier.
//   - Key player rating comparison — average avgRating of each team's 3
//     Key Player Battle picks (a real, globally-comparable SofaScore
//     rating), NOT their compositeScore (which is normalized within each
//     team's own player pool and is not valid for cross-team comparison —
//     verified against the actual computation in queries.ts before using it).
//
// Discipline risk (yellow-card-heavy key players) is surfaced as a plain
// factual note alongside the tally, not folded into it as a penalty —
// "Team A has a key player one booking from suspension" is a fact the
// user can weigh themselves, not a score adjustment we make for them.

export interface CategoryAdvantageInput {
  comparisonRows: Array<{ label: string; homeValue: number | null; awayValue: number | null; higherIsBetter: boolean }>;
  homeAreaVersatility: { DEF: number | null; MID: number | null; FWD: number | null };
  awayAreaVersatility: { DEF: number | null; MID: number | null; FWD: number | null };
  homeKeyPlayerRatings: (number | null)[]; // avgRating of each of home's Key Player Battle picks
  awayKeyPlayerRatings: (number | null)[];
  homeKeyPlayerCardRisk: string[]; // e.g. ['CLOSE TO BAN'] — non-null suspensionRisk values among home's key players
  awayKeyPlayerCardRisk: string[];
}

export interface CategoryAdvantageResult {
  homeLeads: number;
  awayLeads: number;
  even: number;
  totalCategories: number;
  homeLeadLabels: string[];
  awayLeadLabels: string[];
  homeCardRiskNote: string | null; // e.g. "Botafogo-SP: key player CLOSE TO BAN"
  awayCardRiskNote: string | null;
}

export function computeCategoryAdvantageSummary(input: CategoryAdvantageInput): CategoryAdvantageResult {
  const homeLeadLabels: string[] = [];
  const awayLeadLabels: string[] = [];
  let even = 0;

  // Baseline categories — identical edge logic to CategorizedComparison
  // (higherIsBetter polarity respected, null on either side excluded).
  for (const r of input.comparisonRows) {
    if (r.homeValue == null || r.awayValue == null) continue;
    if (r.homeValue === r.awayValue) { even++; continue; }
    const homeBetter = r.higherIsBetter ? r.homeValue > r.awayValue : r.homeValue < r.awayValue;
    if (homeBetter) homeLeadLabels.push(r.label); else awayLeadLabels.push(r.label);
  }

  // Versatility zones — only counted where both sides have data.
  (['DEF', 'MID', 'FWD'] as const).forEach(zone => {
    const h = input.homeAreaVersatility[zone];
    const a = input.awayAreaVersatility[zone];
    if (h == null || a == null) return; // graceful skip, not a 0-fallback — absence isn't a disadvantage
    if (h === a) { even++; return; }
    const label = `${zone} Versatility`;
    if (h > a) homeLeadLabels.push(label); else awayLeadLabels.push(label);
  });

  // Key player rating — average of each team's available Key Player
  // Battle picks' avgRating. Skipped entirely if either side has no
  // rated picks (not defaulted to 0, which would fabricate a disadvantage).
  const avg = (arr: (number | null)[]): number | null => {
    const vals = arr.filter((v): v is number => v != null);
    return vals.length > 0 ? vals.reduce((s, v) => s + v, 0) / vals.length : null;
  };
  const homeKeyAvg = avg(input.homeKeyPlayerRatings);
  const awayKeyAvg = avg(input.awayKeyPlayerRatings);
  if (homeKeyAvg != null && awayKeyAvg != null) {
    if (homeKeyAvg === awayKeyAvg) even++;
    else if (homeKeyAvg > awayKeyAvg) homeLeadLabels.push('Key Player Rating');
    else awayLeadLabels.push('Key Player Rating');
  }

  return {
    homeLeads: homeLeadLabels.length,
    awayLeads: awayLeadLabels.length,
    even,
    totalCategories: homeLeadLabels.length + awayLeadLabels.length + even,
    homeLeadLabels,
    awayLeadLabels,
    homeCardRiskNote: input.homeKeyPlayerCardRisk.length > 0 ? input.homeKeyPlayerCardRisk[0] : null,
    awayCardRiskNote: input.awayKeyPlayerCardRisk.length > 0 ? input.awayKeyPlayerCardRisk[0] : null,
  };
}
