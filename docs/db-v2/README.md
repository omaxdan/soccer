...
| S-0 module-logic recovery | **Complete** | V1 evaluation logic recovered in full · [doc 27](./27-phase8-s0-module-evaluation-recovery.md) |
| Operator runbook | **Current** | Execution order, CLI reference, fresh-database checklist · [doc 28](./28-phase8-execution-runbook.md) |
| Ingestion orchestrator | **Planned, not built** | Blueprint for wiring the unused ingestion writers · [doc 29](./29-phase8-ingestion-orchestrator-plan.md) |
| Backfill & cutover | **Ready to execute** | 2026-05-31 → today, then daily operation · [doc 30](./30-phase8-backfill-and-cutover.md) |
| Supabase compatibility | **Audited** | Migrations 001–020 on managed Supabase · [doc 31](./31supabasecompatibilityaudit.md) |
| Manager / referee / tactical | **Audited** | Feasibility of three intelligence areas · [doc 32](./32managerrefereetacticalaudit.md) |
| Competition context & rotation | **Investigated** | Feasibility of minute/rotation intelligence · [doc 33](./33-phase8-competition-context-investigation.md) |
| Recovery readiness | **Investigated** | Feasibility of next-match response intelligence · [doc 34](./34-phase8-recovery-readiness-investigation.md) |
| Phase 0 provider capability | **BLOCKED — G-1 open** | What the provider integration can actually supply · [doc 35](./35-phase0-provider-capability-investigation.md) |
...
| 27 | [S-0 Module Evaluation Recovery](./27-phase8-s0-module-evaluation-recovery.md) | **The V1 evaluation logic, recovered in full.** Seventeen evaluators — not thirteen — with every threshold, classifier and early return · per-module input inventory · V1→V2 dependency mapping · what S-5 features supply and what is gone · evidence model against `module_evidence_item` · findings S0-4 to S0-10, chiefly that the four single-team evaluators never run and that four V1 modules are retired by decision · four new blockers B-6 to B-9 |
| 28 | [Execution Runbook](./28-phase8-execution-runbook.md) | **How to populate a V2 database from empty.** Execution order with the reason each step precedes the next · exact terminal commands · full CLI reference including undocumented flags and parser behaviour · ingestion order · feature pipeline · numbered fresh-database checklist with expected output and verification SQL · daily run · failure recovery · findings E-1 to E-3, chiefly that only the schedule ingestion job has a runner |
| 29 | [Ingestion Orchestrator Plan](./29-phase8-ingestion-orchestrator-plan.md) | **Implementation blueprint, no code.** How to wire `standings.ts` and `squad.ts` into a production pipeline under a 200-call daily budget · dependency graph · corrected execution order · subcommand CLI · cron cadences drawn from `operations.quality_check` · failure recovery per stage · quota optimisation with work lists derived from ingested data rather than a new table · monitoring · twelve blockers B-1 to B-12 |
| 30 | [Backfill & Production Cutover](./30-phase8-backfill-and-cutover.md) | **Operator execution plan.** Eight phases from prerequisites to daily operation for a 2026-05-31 → today backfill · 66 provider calls, inside budget without an override · why `replay` then `calculate` · thirteen verification queries plus Replay B as the decisive gate · how V2 resumes without a watermark · expected population and what stays empty · risks, chiefly that the window is the close season and a longer backfill is strongly recommended |
| 31 | [Supabase Compatibility Audit](./31supabasecompatibilityaudit.md) | **Audit only, no migration modified.** Migrations 001–020 against PostgreSQL 16 on managed Supabase, verified by full empirical execution · the `must be able to SET ROLE "pt_owner"` failure diagnosed as a PostgreSQL 16 `CREATEROLE` membership change rather than a superuser requirement · one line in migration 001 makes the whole set apply on a non-superuser principal with both conformance gates passing |
| 32 | [Manager, Referee & Tactical Intelligence Audit](./32managerrefereetacticalaudit.md) | **Audit only.** Feasibility of three intelligence areas · the schema is largely already present — `official`, `official_assignment`, `lineup`, `lineup_selection`, `appearance`, `result` all exist · manager is the sole genuine entity gap · almost none of the required data is ingested, S-4 defers all of it by design · the off-season backfill window inverts the build order |
| 33 | [Competition Context & Rotation Intelligence](./33-phase8-competition-context-investigation.md) | **Feasibility investigation, nothing implemented.** Twenty capabilities classified · why the season-minute delta approach is superseded by `football.appearance` · complete data flow and where it stops · API capability audit separating available / not ingested / unknown · finding CC-1, that cup fixtures already contaminate league form · fourteen ranked data gaps · false-positive and false-negative risks · nine phases · acceptance criteria including the Rosario Central test |
| 34 | [Recovery Readiness](./34-phase8-recovery-readiness-investigation.md) | **Feasibility investigation, nothing implemented.** Sixteen capabilities classified · `calibration_result` already models the measured-rate-with-sample shape, but is not team-scoped, so the values belong in `feature_value` · V2 has no team strength model and no xG, so opponent strength must be built from `football.result` · the two circularity classes and which one to avoid · why full segmentation needs 6+ seasons and what depth to fix instead · shrinkage toward a baseline alongside the existing hard gate · orthogonality to Recent Form · the eight test cases and what each gates on · needs no new relation and no new column |
| 35 | [Phase 0 — Provider Capability](./35-phase0-provider-capability-investigation.md) | **Read-only, decision OPTION C — BLOCKED.** The complete 13-path provider surface across both codebases, each marked EXISTS+CALLED / EXISTS+UNCALLED / none · the full trace from endpoint to table for `appearance`, `lineup`, `lineup_selection`, all broken at the first link · `/match/{id}` registered and never called, its payload unknown · season aggregates confirmed and confirmed insufficient · AC-4 assessed requirement by requirement · LIVE PAYLOAD NOT OBTAINED, no credentials, nothing fabricated · G-4 and G-9 confirmed not to depend on G-1 |
